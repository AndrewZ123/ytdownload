...
function scanLibrary(callback) {
  if (libraryCache && Date.now() - libraryCacheTime < LIBRARY_CACHE_TTL) {
    return callback(null, libraryCache);
  }
        ...
        execFile('ffprobe', ['-hide_banner', '-print_format', 'json', '-show_format', filePath],
          { maxBuffer: 2 * 1024 * 1024, timeout: 5000 }, (err, stdout) => {
            ...
            if (pending === 0 && done) {
              ...
              libraryCache = { songs, artists: Object.values(artists), albums: Object.values(albums) };
              libraryCacheTime = Date.now();
              callback(null, libraryCache);
            }
          });
      }
    }
...
app.get('/api/music/cover/:playlist/:file', (req, res) => {
  ...
  const coverPath = path.join(coverDir, hashStr(playlist + '/' + file) + '.jpg');
  ...
  execFile('ffmpeg', ['-i', filePath, '-an', '-vcodec', 'copy', '-y', coverPath],
    { timeout: 5000 }, (err) => {
      if (err || !fs.existsSync(coverPath)) {
        // Return a default 1x1 transparent pixel
        const pixel = Buffer.from('/9j/4AAQSkZJRgABAQEASABIAAD/2wBDAAgGBgcGBQgHBwcJCQgKDBQNDAsLDBkSEw8UHRofHh0aHBwgJC4nICIsIxwcKDcpLDAxNDQ0Hyc5PTgyPC4zNDL/wAALCAABAAEBAREA/8QAFAABAAAAAAAAAAAAAAAAAAAACf/EABQQAQAAAAAAAAAAAAAAAAAAAAD/2gAIAQEAAD8AKp//2Q==', 'base64');
...
// ==================== Streaming Endpoints (for iOS app) ====================
app.get('/api/music/stream', (req, res) => {
  ...
  const mimeTypes = { '.mp3': 'audio/mpeg', '.m4a': 'audio/mp4', '.flac': 'audio/flac', '.wav': 'audio/wav', '.ogg': 'audio/ogg', '.opus': 'audio/opus', '.aac': 'audio/aac' };
...
app.get('/api/music/download', (req, res) => {
  const { playlist, file } = req.query;
  if (!playlist || !file) return res.status(400).json({ error: 'Missing playlist or file' });

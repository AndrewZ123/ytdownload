 ...
 */
  function fetchUpstream(url, headers = {}, maxRedirects = 8, timeoutMs = 30000, useWarp = true) {
  return new Promise((resolve, reject) => {
    function attempt(currentUrl, remaining) {
      ...
      // Use WARP for streaming to maintain the same IP as the resolver
      // YouTube CDN URLs are IP-bound to the resolver's IP (WARP IP)
      // If we fetch without WARP, YouTube sees a different IP and returns 403
      let usingWarp = false;
      if (useWarp && isWarpAvailable()) {
        opts.agent = getWarpAgent();
        usingWarp = true;
        console.log(`[fetchUpstream] Using WARP SOCKS5 proxy for ${parsed.hostname}`);
      } else {
        // Fall back to connection pooling for non-WARP requests
        opts.agent = isHttps ? httpsAgent : httpAgent;
        console.log(`[fetchUpstream] Using direct connection for ${parsed.hostname} (WARP ${useWarp ? 'requested but' : 'not'} available)`);
      }
      ...
      const req = lib.request(opts, (res) => {
        ...
        console.log(`[fetchUpstream] Response from ${parsed.hostname}: ${res.statusCode} (using WARP: ${usingWarp})`);
        resolve(res);
      });

      req.on('error', (err) => {
        console.error(`[fetchUpstream] Request error for ${parsed.hostname} (using WARP: ${usingWarp}):`, err.message);
        reject(err);
      });
      req.on('timeout', () => { 
        console.error(`[fetchUpstream] Request timeout for ${parsed.hostname} (using WARP: ${usingWarp})`);
        req.destroy(); 
        reject(new Error('Upstream fetch timeout')); 
      });
      req.end();
    }
...
async function proxyStream(resolution, req, res, tokenAge) {
  ...
  // TRUST the resolver's audioMime field - it comes from yt-dlp's format detection
  let responseContentType = audioMime || 'audio/mp4';
  
  // Only use upstream content-type if it's specific (not generic octet-stream)
  ...
  // SAFETY CHECK: Never serve WebM to iOS - it will fail with SRC_NOT_SUPPORTED
  if (responseContentType === 'audio/webm' || responseContentType === 'audio/webm; codecs=opus' || 
      responseContentType === 'audio/webm; codecs=vorbis') {
    console.error(`[stream-proxy] ❌ CRITICAL: Refusing to serve WebM format to iOS for ${sourceId}`);
    console.error(`[stream-proxy] audioMime=${audioMime}, upstreamContentType=${upstreamContentType}`);
    streamLog.error = `unsafe_format_webm_for_ios`;
    streamLog.downstreamStatus = 502;
    _logStreamSession(streamLog);
    if (!res.headersSent) {
      res.status(502).json({ 
        error: 'Stream format not supported by iOS', 
  ...
  // Ensure we always send a valid iOS-compatible content-type
  if (!responseContentType || responseContentType === 'application/octet-stream') {
    responseContentType = 'audio/mp4';
  }
  
  console.log(`[stream-proxy] Streaming ${sourceId} with Content-Type: ${responseContentType}`);

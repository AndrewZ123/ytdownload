...
// Performance improvement #4: Redis integration
// Try to initialize Redis connection (non-blocking)
redisCache.init().catch(() => {
  console.log('[resolver] Redis not available, using in-memory cache only');
});

// ==================== Cache Operations ====================

async function getCachedResolution(videoId) {
  // 1. Check Redis first (Performance improvement #4)
  if (redisCache.isAvailable()) {
    const redisCached = await redisCache.getCachedResolution(videoId);
    if (redisCached) {
      // Populate local cache too for faster subsequent access
      resolverCache.set(videoId, redisCached);
      console.log(`[resolver] Redis cache hit for ${videoId}`);
      return redisCached;
    }
  }
  ...
  if (cached) {
    // Parse ISO string to timestamp for comparison
    const resolvedTime = new Date(cached.resolvedAt).getTime();
...
async function setCachedResolution(videoId, data) {
  // Don't overwrite resolvedAt - it's already set by _runYtDlp as an ISO string
  const cacheEntry = {
    ...data,
  };

  // Set in-memory cache
  resolverCache.set(videoId, cacheEntry);

  // Also set in Redis (Performance improvement #4)
  if (redisCache.isAvailable()) {
    await redisCache.setCachedResolution(videoId, cacheEntry);
  }
  ...
  if (resolverCache.size > 500) {
    ...
    for (const [key, val] of resolverCache) {
      const resolvedTime = new Date(val.resolvedAt).getTime();
      if (now - resolvedTime > RESOLVER_CACHE_TTL) resolverCache.delete(key);
    }
  }
}
 ...
 */
async function resolve(videoId, options = {}) {
  const retries = options.retries ?? 1;

  // 1. Check cache (Redis + memory)
  const cached = await getCachedResolution(videoId);
  if (cached) {
...
function getStats() {
  return {
    ...
    entries: Array.from(resolverCache.entries()).map(([id, v]) => {
      ...
      return {
        videoId: id,
        age: Math.round((Date.now() - resolvedTime) / 1000),
        expired: Date.now() - resolvedTime > RESOLVER_CACHE_TTL,
      };
    }),
  };
}

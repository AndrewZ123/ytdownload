...
function _resolveWithRetry(videoId, retriesLeft) {
  return _runYtDlp(videoId).catch(err => {
    if (retriesLeft > 0) {
      console.warn(`[resolver] Retrying ${videoId} (${retriesLeft} left): ${err.message}`);
      return new Promise(resolve => setTimeout(resolve, 1500)).then(() => _resolveWithRetry(videoId, retriesLeft - 1));
    }
    throw err;
  });
}
  ...
  return new Promise((resolve, reject) => {
    ...
    ytdlp.on('close', code => {
      ...
      try {
        ...
        console.log(`[resolver] Resolved ${videoId} → ${upstreamUrl.slice(0, 100)}... (${audioMime}, ${bitrate}bps)`);
